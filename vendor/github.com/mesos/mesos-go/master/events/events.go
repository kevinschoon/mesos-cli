...
type (
	...
	Handler interface {
		HandleEvent(*master.Event) error
	}

	// HandlerFunc is a functional adaptation of the Handler interface
	HandlerFunc func(*master.Event) error
	...
	Mux struct {
		handlers       map[master.Event_Type]Handler
...
func (f HandlerFunc) HandleEvent(e *master.Event) error { return f(e) }

// NewMux generates and returns a new, empty Mux instance.
func NewMux(opts ...Option) *Mux {
	m := &Mux{
		handlers: make(map[master.Event_Type]Handler),
	}
	m.With(opts...)
	return m
}
...
func (m *Mux) HandleEvent(e *master.Event) (err error) {
...
func Handle(et master.Event_Type, eh Handler) Option {
