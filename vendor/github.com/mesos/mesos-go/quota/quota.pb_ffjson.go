...

	{
		...
		if tok == fflib.FFTok_null {
			...
			for {
				...
				/* handler: tmp_uj__Guarantee type=*mesos.Resource kind=ptr quoted=false*/

				{
					if tok == fflib.FFTok_null {

						tmp_uj__Guarantee = nil

						state = fflib.FFParse_after_value
						goto mainparse
					}

					if tmp_uj__Guarantee == nil {
						tmp_uj__Guarantee = new(mesos.Resource)
					}
...

	{
		...
		if tok == fflib.FFTok_null {
			...
			for {
				...
				/* handler: tmp_uj__Infos type=*quota.QuotaInfo kind=ptr quoted=false*/

				{
					...
					if tmp_uj__Infos == nil {
						tmp_uj__Infos = new(QuotaInfo)
					}
